...
public class UserController {
    ...
    public ResponseEntity<Void> addUser(@RequestBody User user) {
        users.add(user); return ResponseEntity.accepted().build();
    }
    ...
    public ResponseEntity<Void> deleteUser(@PathVariable int index) { 
        users.remove(index); return ResponseEntity.noContent().build();
    }

    //curl -X PUT -H "Content-Type: application/json" -d "30" http://localhost:8080/users/0/age
    @PutMapping("users/{index}/age")
    public ResponseEntity<Void> updateAge(@PathVariable int index, @RequestBody int age) { 
        users.get(index).setAge(age); return ResponseEntity.accepted().build();
    }
}